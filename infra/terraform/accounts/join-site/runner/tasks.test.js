...
const { roleOracles } = require('./role-oracles.testdata')
...
test('five new direction assignments are distinct and mobile requires both languages', () => {
  ...
  assert.deepEqual(current.map(([, taskSet]) => taskSet.direction).sort(),
    ['Бэкенд', 'Фронтенд', 'Мобильная разработка', 'AI/ML', 'Системная разработка'].sort())
  const ids = []
...
test('v4 role assignments use only Go/Python and keep product-specific native references', () => {
  for (const [version, language] of [['edium-go-2026-09-v4-backend', 'go'], ['edium-python-2026-09-v4-ai-ml', 'python']]) {
    const taskSet = catalog.sets[version]
    assert.deepEqual(taskSet.languages, [language])
    assert.equal(taskSet.tasks.length, 3)
    for (const task of taskSet.tasks) {
      assert.deepEqual(Object.keys(task.languages), [language])
      assert.equal(task.defaultLanguage, language)
      assert.equal(task.tests.length, 7)
      assert.equal(task.publicExampleCount, 2)
      assert.equal(new Set(task.tests.map(item => item.name)).size, 7)
      assert.equal(typeof roleReferenceSolutions[task.id][language], 'string')
      assert.equal(typeof roleOracles[task.id], 'function')
      assert.match(task.languages[language].starterCode, language === 'go' ? /package main/ : /def solve\(data: dict\)/)
    }
  }
  assert.equal(getTask('edium-js-2026-09-v3-backend', 'backend-webhook-receipts'), undefined)
  assert.equal(getTask('edium-go-2026-09-v4-backend', 'notification-bouncer'), undefined)
})
...
for (const [version, taskSet] of Object.entries(catalog.sets)) {
  ...
  for (const task of taskSet.tasks) {
    test(taskSet.direction + ': ' + task.title + ' — every fixture agrees with the reference', async () => {
      const solve = referenceSolutions[task.id] || roleOracles[task.id]
      ...
      // Non-JS references are compiled and executed by the native-language
